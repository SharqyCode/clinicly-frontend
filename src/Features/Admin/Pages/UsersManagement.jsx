...
import { getAllDoctors, getAllPatients,deleteDoctor,deletePatient ,updateDoctor,updatePatient,getAllReseptionists,updateReceptionist,deleteReceptionist} from "../../../Api/Services/UserMangementService";
...
const updateDoctorMutation = useMutation({
  mutationFn: ({ doctorId, updateData }) => updateDoctor(doctorId, updateData),
  onSuccess: () => {
    queryClient.invalidateQueries(["doctors"]);
    setStatus("success");
  },
...
const updatePatientMutation = useMutation({
  mutationFn: ({ patientId, updateData }) => updatePatient(patientId, updateData),
  onSuccess: () => {
    queryClient.invalidateQueries(["patients"]);
    setStatus("success");
  },
...
const updateReceptionistMutation = useMutation({
  mutationFn: ({ receptionistId, updateData }) => updateReceptionist(receptionistId, updateData),
  onSuccess: () => queryClient.invalidateQueries(["receptionists"]),
});
...
<div className="flex flex-col sm:flex-row sm:items-center gap-4 p-2 border-b hover:bg-gray-50 rounded-lg transition w-full">
  ...
  <div className="w-14 h-14 flex-shrink-0">
    <img
      src={doc.userId.profileImage || "/default-avatar.png"} // fallback image
  ...
  <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4 flex-1 min-w-0">
    ...
    <div className="text-gray-600 truncate">{doc.specialization}</div>
    <div className="text-gray-500 text-sm truncate">{doc.userId.email}</div>
    <div className="text-gray-500 text-sm truncate">{doc.userId.phone || "N/A"}</div>
  </div>
  ...
  {showReceptionists && (
    <div className="space-y-2 mt-2">
      {receptionistsLoading ? (
        ...
        receptionistsData.data.map((rec) => (
          rec.isActive &&
          <div key={rec._id} className="flex justify-between items-center border-b p-2">
            <div>
              {rec.firstName} {rec.lastName} — {rec.email} — {rec.phone}
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => {
              ...
              <button
                onClick={() => deleteReceptionistMutation.mutate(rec._id)}
                className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600"
              >
                Delete
              </button>
...
<UpdatePatientModal
  ...
  onSubmit={(data) =>
    updatePatientMutation.mutate({ patientId: selectedPatient._id, updateData: data })
  }
/>
<UpdateReceptionistModal
  ...
  onSubmit={(data) => {
    updateReceptionistMutation.mutate({ receptionistId: selectedReceptionist._id, updateData: data });
